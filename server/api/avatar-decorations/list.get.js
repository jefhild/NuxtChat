...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (error) {
      console.error("[avatar-decorations.list] storage list error:", error);
    ...
    if (storageNames.length) {
      ...
      if (metadataError) {
        console.warn(
          "[avatar-decorations.list] metadata lookup warning:",
          metadataError
        );
      } else {
    ...
    console.error("[avatar-decorations.list] error:", err);
    setResponseStatus(event, 500);
    return {
