...
export default defineEventHandler(async (event) => {
  try {
    ...
    if (meErr) {
      console.error("[admin/profile-photos.approve] admin check error:", meErr);
    ...
    if (error) {
      console.error("[admin/profile-photos.approve] update error:", error);
      setResponseStatus(event, 500);
    ...
    console.error("[admin/profile-photos.approve] error:", err);
    setResponseStatus(event, 500);
    return {
