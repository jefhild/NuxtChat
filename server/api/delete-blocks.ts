export default defineEventHandler(async (event) => {
  ...
  const deleteResult = await deleteAllBlocks(config.SENDGRID_API_KEY);
...
async function deleteAllBlocks(sendgridApiKey: string) {
  const deleteUrl = `https://api.sendgrid.com/v3/suppression/blocks`;
