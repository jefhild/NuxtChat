export default defineEventHandler(async (event) => {
  ...
  // Step 1: Optionally, verify the email is in the blocks list
  ...
  await deleteBlock(email, config);
...
async function deleteBlock(email, config) {
  const deleteUrl = `https://api.sendgrid.com/v3/suppression/blocks/${encodeURIComponent(
    email
  )}`;
  ...
  try {
    ...
    console.error(`Error deleting block for email ${email}:`, error);
    return null; // Return null if there's an error during deletion
  }
}
