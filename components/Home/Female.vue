  ...
  <v-container fluid>
    <v-expansion-panels flat v-model="activePanels">
        ...
        <v-expansion-panel-text>
          <v-row no-gutters>
            ...
            <v-col
              cols="12"
              md="3"
              v-for="(profile, index) in recentProfiles.slice(0, 4)"
      ...
      <v-col class="text-right">
        <NuxtLink
          href="/profiles/female"
          class="text-body-2 font-style-poppins mr-5"
          >See More Female Profiles</NuxtLink
        >
      </v-col>
    </v-row>
  </v-container>
...
const profileLimit = 4;

const { getRecentFemales } = useDb();

const data = getRecentFemales(profileLimit);
