		...
		<v-card>
			...
			<v-card-text v-else>
				<v-form ref="editForm" @submit.prevent="handleUpdate">
					<v-text-field v-model="selectedCategory.name" label="Category Name"
						:rules="[v => !!v || 'Name is required']" />
				</v-form>
			</v-card-text>
			<v-card-actions>
				<v-btn :disabled="loadingUpdate" color="primary" :to="`/categories/${selectedCategory.slug}`"
					align-content-start>
					Go to Category Page
				</v-btn>
				<v-spacer></v-spacer>
...
const handleSubmit = async () =>
{
	...
	try
	{
		...
		const duplicate = categories.value.find(cat =>
			cat.name === formattedName || cat.slug === generatedSlug
		);
