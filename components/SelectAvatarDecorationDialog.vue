<template>
	<v-card>
		...
		<v-card-text>
			...
			<v-row>
				<v-col v-for="decoration in allAvatarDecorations" :key="decoration.name" cols="4"
					class="d-flex flex-column align-center">
					<div class="decoration-option" :class="{ selected: decoration.url === selectedDecoration }"
						@click="handleDecorationClick(decoration.url)">

						<div class="photo-container">
							<NuxtImg v-if="photopath" :src="photopath" class="cover-image" />
							<NuxtImg v-if="decoration.url" :src="decoration.url" width="80" height="80" class="avatar-decoration" />
						</div>
					</div>
