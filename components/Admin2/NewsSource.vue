...
const { previewManualRewrite, rewriteManual, saveUrlDraft } = useAdminNewsSource();
const { init: initMarkdown, render: mdRender } = useMarkdown();
const { getAllCategories, getAllPeople, getAllTags } = useDb();
...
const botOptions = computed(() =>
  activeBots.value.map((bot) => ({
    label: `${bot.profile?.displayname || bot.persona_key} (${
      bot.category?.name || bot.model
    })`,
...
const peopleOptions = ref<Array<{ id?: number; name?: string }>>([]);
...
const loadPeople = async () => {
  ...
  try {
    ...
    peopleOptions.value = Array.isArray(response) ? response : [];
  } catch (error: any) {
    console.warn("[NewsSource] loadPeople error", error);
    manualError.value = error?.message || "Unable to load people right now.";
  } finally {
...
const loadTags = async () => {
  ...
  try {
    ...
    tagOptions.value = Array.isArray(response) ? response : [];
  } catch (error: any) {
    console.warn("[NewsSource] loadTags error", error);
    manualError.value = error?.message || "Unable to load tags right now.";
  } finally {
...
const loadBots = async () => {
  ...
  try {
    ...
    manualError.value = error?.message || "Failed to load AI bots.";
  } finally {
